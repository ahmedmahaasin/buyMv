...
import { assets } from "../../assets/assets"; // parcel icon

const ConfirmedOrders = ({ token }) => {
  ...
  const fetchConfirmedOrders = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        const confirmed = res.data.orders.filter(order => order.status.toLowerCase() === "confirmed");
        setOrders(confirmed);
      } else {
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <h2 className="text-3xl font-bold mb-6">Confirmed Orders</h2>
      ...
      {selectedOrder && (
        <OrderModal
          ...
          onStatusUpdate={fetchConfirmedOrders} // refresh confirmed orders after update
        />
      )}
    </div>
  );
