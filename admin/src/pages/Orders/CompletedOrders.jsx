...
const CompletedOrders = ({ token }) => {
  ...
  const fetchCompletedOrders = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        const completed = res.data.orders.filter(order => order.status.toLowerCase() === "completed");
        setOrders(completed);
      } else {
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <h2 className="text-3xl font-bold mb-6">Completed Orders</h2>
      ...
      {selectedOrder && (
        <OrderModal
          ...
          onStatusUpdate={fetchCompletedOrders} // refresh completed orders after update
        />
      )}
    </div>
  );
