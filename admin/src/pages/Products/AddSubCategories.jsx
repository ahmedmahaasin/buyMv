...
const AddSubCategories = ({ token }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!catName || !catImage) {
      toast.error("Subcategory name and image are required!");
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      if (res.data.success) {
        ...
        toast.error("Failed to add subcategory: " + res.data.message);
      }
    } catch (error) {
  ...
  return (
    <>
      ...
      >
        ...
        {/* Subcategory Image */}
        <div className="relative">
          <label className="block mb-2 font-semibold text-gray-700">Subcategory Image</label>
          <label className="cursor-pointer border-2 border-dashed border-gray-300 rounded-xl flex items-center justify-center h-64 hover:border-blue-500 transition">
          ...
          {catImage && (
            <button
              ...
              className="absolute top-10 right-2 bg-red-500 text-white px-2 py-1 rounded-full hover:bg-red-600 transition text-xs"
            >
              Remove
            </button>
          )}
